...
class TestEcodTGroupValidation:
    ...
    @pytest.mark.slow
    def test_8ovp_A_tgroup_assignment(
        ...
        # Run the algorithm
        result = self._run_mini_algorithm(
            protein_id, stable_batch_dir, real_reference_data, blast_alignments
        )
        ...
        assert tgroup_validation["checks"][
            "discontinuous_architecture"
        ], "Missing discontinuous architecture"
        assert tgroup_validation["checks"]["coverage"], "Insufficient coverage"

        print("\n✅ ECOD T-GROUP VALIDATION PASSED")
        print("   Mini correctly assigns domains to ECOD T-groups")
        print("   Biological architecture properly detected")
        print("   Production-quality results achieved")

    def _run_mini_algorithm(
        self, protein_id: str, batch_dir: str, reference_data: dict, blast_alignments: dict
    ) -> dict:
        """Run mini algorithm and return results"""
        import os

        parts = protein_id.split("_")
        _pdb_id, _chain_id = parts[0], parts[1] if len(parts) > 1 else "A"
    ...
    def _validate_tgroup_assignments(self, domains: list, ecod_classifications: dict) -> dict:
        ...
        validation = {"checks": {}, "tgroup_details": []}
        ...
        for domain in domains:
            ...
            if hasattr(domain, "evidence_items") and domain.evidence_items:
                t_group = (
                    domain.evidence_items[0].t_group
                    if domain.evidence_items[0].t_group
                    else "unknown"
                )
            else:
                t_group = "unknown"
    ...
    def _validate_domain_characteristics(self, domains: list) -> dict:
        ...
        validation["checks"]["discontinuous_architecture"] = any(
            d.range.is_discontinuous for d in domains
        )
        validation["checks"]["coverage"] = sum(d.range.total_length for d in domains) >= 450
        validation["checks"]["decomposition_success"] = any(
            d.source == "chain_blast_decomposed" for d in domains
        )
