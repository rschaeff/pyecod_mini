...
class CoordinateTranslator:
    ...
    def translate_seqid_range(self, seqid_start: int, seqid_end: int) -> tuple[str, str]:
        ...
        if pdb_start is None or pdb_end is None:
            print(
                f"Warning: Could not translate seqid range {seqid_start}-{seqid_end} "
    ...
    def get_mapping_info(self) -> dict:
        ...
        return {
            ...
            "seqid_range": (
                f"{min(self.seqid_to_pdb.keys())}-{max(self.seqid_to_pdb.keys())}"
                if self.seqid_to_pdb
                else "None"
            ),
            "pdb_range": (
                f"{min(self.pdb_to_seqid.keys(), key=lambda x: int(re.sub(r'[A-Z]', '', x)))}-{max(self.pdb_to_seqid.keys(), key=lambda x: int(re.sub(r'[A-Z]', '', x)))}"
                if self.pdb_to_seqid
                else "None"
            ),
            "has_insertion_codes": any(not x.isdigit() for x in self.pdb_to_seqid),
        }
...
class PyMOLVisualizer:
    ...
    def create_comparison(
        ...
        output_dir: str = "/tmp/pymol_comparison",
    ) -> str:
        """
        ...
        if not structure_path:
            msg = f"Structure not found for {pdb_id}"
    ...
    def _translate_domain_coordinates(
        self, domains: list[Domain], translator: CoordinateTranslator
    ) -> list[Domain]:
        ...
        try:
            ...
            for i, domain_elem in enumerate(root.findall(".//domain")):
                if is_old_format:
                    ...
                    family = domain_elem.get("source_id", "") or domain_elem.get(
                        "t_group", "unknown"
                    )
                ...
                domains.append(
                    Domain(id=domain_id, range_str=range_str, family=family, segments=segments)
                )
    ...
    def _write_pymol_script(
            ...
            if domain.pdb_segments:
                for pdb_start, pdb_end in domain.pdb_segments:
                    ...
                    if pdb_start == pdb_end:
                        ...
                        selection = (
                            f"old_algorithm and chain {chain_id} and resi {pdb_start}-{pdb_end}"
                        )
                    lines.append(f"color {color}, {selection}")
                    lines.append(f"# PDB coordinates: {pdb_start}-{pdb_end}")
            else:
                lines.append("# Warning: No PDB coordinates available for this domain")
            ...
            if domain.pdb_segments:
                for pdb_start, pdb_end in domain.pdb_segments:
                    if pdb_start == pdb_end:
                        ...
                        selection = (
                            f"new_algorithm and chain {chain_id} and resi {pdb_start}-{pdb_end}"
                        )
                    lines.append(f"color {color}, {selection}")
                    lines.append(f"# PDB coordinates: {pdb_start}-{pdb_end}")
            else:
        ...
        lines.extend(
            [
                ...
                "print 'Coordinate translation summary:'",
    ...
    def quick_comparison(self, protein_id: str, batch_dir: str) -> str:
        ...
        if not os.path.exists(old_file):
            msg = f"Old domains file not found: {old_file}"
            raise FileNotFoundError(msg)
        if not os.path.exists(new_file):
            msg = f"New domains file not found: {new_file}"
            raise FileNotFoundError(msg)
...
def create_comparison(
    ...
    return visualizer.create_comparison(protein_id, old_domains_file, new_domains_file, output_dir)


def quick_comparison(
    protein_id: str, batch_dir: str = "/data/ecod/pdb_updates/batches/ecod_batch_036_20250406_1424"
) -> str:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create PyMOL domain comparison with coordinate translation"
    )
    ...
    parser.add_argument("--output-dir", default="/tmp/pymol_comparison", help="Output directory")

    args = parser.parse_args()

    try:
        if args.old_file and args.new_file:
            script_path = create_comparison(
                args.protein_id, args.old_file, args.new_file, args.output_dir
            )
        else:
