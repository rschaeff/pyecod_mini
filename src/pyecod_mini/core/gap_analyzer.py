...
from .models import Domain, SegmentType, UnassignedSegment
from .sequence_range import SequenceRange


def find_unassigned_segments(
    domains: list[Domain], sequence_length: int, min_domain_size: int = 25
) -> list[UnassignedSegment]:
    """
...
def get_gap_analysis_summary(segments: list[UnassignedSegment], sequence_length: int) -> dict:
    ...
    coverage_percent = (
        ((sequence_length - total_unassigned) / sequence_length * 100) if sequence_length > 0 else 0
    )
...
def validate_gap_analysis(
    segments: list[UnassignedSegment], domains: list[Domain], sequence_length: int
) -> list[str]:
    """
    ...
    for segment in segments:
        for domain in domains:
            if segment.range.overlaps(domain.range):
                ...
                issues.append(
                    f"Segment {segment.start}-{segment.end} overlaps with domain {domain.id} at positions {sorted(overlap_positions)}"
                )
    ...
    for i, seg1 in enumerate(segments):
        for seg2 in segments[i + 1 :]:
            if seg1.range.overlaps(seg2.range):
                ...
                issues.append(
                    f"Segments {seg1.start}-{seg1.end} and {seg2.start}-{seg2.end} overlap at positions {sorted(overlap_positions)}"
                )
...
def perform_complete_gap_analysis(
    domains: list[Domain],
    sequence_length: int,
    ...
    validate: bool = True,
    verbose: bool = False,
) -> tuple[list[UnassignedSegment], dict]:
    """
    ...
    if verbose:
        for segment in segments:
            print(
                f"  Segment {segment.start}-{segment.end}: length={segment.length}, "
